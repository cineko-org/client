...
export function useMonitorCommands(
  ...
  const executeRetry = useCallback(async (id: string) => {
    ...
    setMutationId(id);
    try {
      await api('/api/monitors/retry', { method: 'POST', body: { id, userId, headful: true } });
  ...
  const retry = useCallback((id: string) => {
    ...
    if (monitor?.status !== 'triggered' && monitor?.status !== 'payment_unknown') return;
  ...
  const remove = useCallback(async () => {
	...
	try {
		const monitor = monitors.find((item) => item.id === id);
		await api('/api/monitors', { method: 'DELETE', body: { id, userId, revision: monitor?.revision ?? 0 } });
      await reload();
      notify('모니터를 삭제했습니다.');
	} catch (error) {
	...
	}, [deleteId, monitors, notify, reload, userId]);

  return {
    deleteId, setDeleteId, retryMonitor: monitors.find((monitor) => monitor.id === retryId),
