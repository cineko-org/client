...
readonly signer='scripts/sign-notarize-macos-client.sh'
readonly release_config='release-please-config.json'

fail() {
  printf 'macOS Client signing workflow check failed: %s\n' "$1" >&2
  exit 1
}
...
[[ -n "$sign_line" && -n "$notary_line" && -n "$staple_line" && -n "$final_zip_line" ]] || \
  fail 'release operation order cannot be verified'
