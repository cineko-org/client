...
# Signs the macOS Client, notarizes it with Apple, staples the accepted ticket,
# and emits the only archive that the release workflow may publish.
...
fail() {
  printf 'macOS Client signing failed: %s\n' "$1" >&2
  exit 1
}
...
printf 'Signed and notarized macOS Client: %s\n' "$output_path"
