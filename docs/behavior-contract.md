...
| Catalog | `GET /v1/catalog`, `GET /v1/catalog/auditoriums/{auditoriumId}/seat-map`, `POST /v1/catalog/auditoriums/{auditoriumId}/seat-map:resolve` | Client requests a layout by auditorium ID only. Central returns its stored current layout immediately or returns `waiting` while arranging collection behind the boundary. Client has no catalog or seat-layout write service point. | Missing layout never starts a Client-side scan. The Client does not select dates, Probe instances, provider pages, or refresh policy. |
...
| Resolve seat map | `POST /api/catalog/seat-map` | Sends only `auditoriumId` to Central and renders the returned stored layout | `202 waiting` keeps preset editing available; Client never accepts dates, force flags, or Probe controls |
