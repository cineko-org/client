...
| Catalog | `GET /v1/catalog`, `POST /v1/catalog/auditoriums/{auditoriumId}/seat-map:resolve`, `GET /v1/catalog/auditoriums/{auditoriumId}/seat-map:watch` | Client requests or watches a layout by auditorium ID only. Central immediately streams the cached layout and current generated collection state, then emits only durable changes. Client cannot force catalog or layout collection. | Missing layout never starts a Client-side scan or polling loop. The Client does not select dates, Probe instances, provider pages, or refresh policy. |
| Live seats | `POST /v1/catalog/live-seat-observations` | Client sends the generated `SubmitLiveSeatObservationRequest`; `Idempotency-Key` exactly equals its content-addressed `mutation.commandId` and append semantics require `expectedRevision=0`. Layout plus availability, including zero available seats, remain one atomic observation. | Client validates and ranks its live browser observation, prepares the provider payment handoff, durably stores that handoff, and only then performs the report with a two-second request deadline. A cancellation-seat watch does not report between hot retries: success reports after the hold, while terminal exhaustion reports the latest valid zero-seat observation once. Report failure cannot discard a successful provider seat hold; Central's returned snapshot is not substituted into the live ranking input. |
...
- Seat-hold critical-path accounting removes one Central round trip: previously the live observation report completed before provider payment preparation; now provider payment preparation completes first and the bounded report follows the durable handoff. A sold-out cancellation watch also performs zero reports across its hot retries and only one terminal report after exhaustion. External-network wall time is not a stable local benchmark, so ordering and failure tests prove these critical boundaries directly, including a report that blocks until its deadline without invalidating the prepared reservation.
...
- Client, browser, and Playwright metadata are separate compatible release components. The Client publisher receives the minimum Launcher version plus the browser revision and Playwright version produced by the locked runtime workflows, then validates the generated release set and wraps it in the exact latest `PublishClientRequest`, `PublishBrowserRequest`, or `PublishPlaywrightRequest` service message. Shell and `jq` may not author release-registry wire JSON. Each success strictly decodes the matching generated response, rejects empty/unknown bodies, and requires a positive release-generation header. A Client release first ensures the immutable Playwright runtime and its official Chrome for Testing manifest are registered, then builds and registers the Client. Existing runtime assets and manifests are reused and never overwritten.
- The generated publisher selects only `/v1/release-registry/client`, `/v1/release-registry/browser`, or `/v1/release-registry/playwright` below the common `/v1/release-registry/` service point; arbitrary components and paths are rejected before network I/O.
