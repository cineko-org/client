...
func TestConfigFromLookup(t *testing.T) {
	...
	if !reflect.DeepEqual(config.ScanProxies, wantProxies) {
		t.Fatalf("scan proxies = %+v, want %+v", config.ScanProxies, wantProxies)
	}
}
...
func TestConfigFromLookupRejectsInvalidValues(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "proxy", key: "CINEKO_SCAN_PROXIES", value: "ftp://127.0.0.1:21"},
	} {
...
func TestSoxyClientRejectsInvalidOriginAndMalformedResponses(t *testing.T) {
	t.Parallel()
	for _, rawURL := range []string{
		"", "ftp://soxy.test", "http://soxy.test", "https://user@soxy.test", "https://soxy.test/path",
		"https://soxy.test?q=1",
	} {
...
func TestSoxyClientAllowsPlainHTTPOnlyForExplicitLoopback(t *testing.T) {
	t.Parallel()
	for _, rawURL := range []string{"http://localhost:8080", "http://127.0.0.1:8080", "http://[::1]:8080"} {
		if _, err := newSoxyClient(rawURL, "token", nil); err != nil {
			t.Errorf("newSoxyClient(%q) error = %v", rawURL, err)
		}
	}
	if _, err := newSoxyClient("https://soxy.example.test", "token", nil); err != nil {
		t.Fatalf("newSoxyClient(HTTPS) error = %v", err)
	}
}

func TestProxyAndSoxyParseErrorsDoNotExposeCredentials(t *testing.T) {
	t.Parallel()
	const secret = "never-log-this-password"
	for name, test := range map[string]func() error{
		"proxy": func() error {
			_, err := ParseProxy("http://user:" + secret + "@example.test:8080/%zz")
			return err
		},
		"soxy": func() error {
			_, err := newSoxyClient("https://user:"+secret+"@example.test/%zz", "token", nil)
			return err
		},
	} {
		err := test()
