...
// These exact macOS ARM64 Stable builds were verified against Google's public
// Chrome VersionHistory API on 2026-08-09. Legacy UA strings are reduced to the
// major version, while UA-CH receives the corresponding real full version.
var verifiedScanChromeVersions = []string{
	"151.0.7922.109",
	"150.0.7871.189",
	"149.0.7827.201",
}
...
func selectBrowserUserAgent(chromePath string, mode UserAgentMode, random io.Reader) (browserUserAgent, error) {
	...
	for _, version := range verifiedScanChromeVersions {
...
func readNativeUserAgentIdentity(
	...
	if !replaceChromiumVersion(identity.Brands, selected.Major) ||
		!replaceChromiumVersion(identity.FullVersionList, selected.FullVersion) {
...
func replaceChromiumVersion(brands []uaBrandVersion, version string) bool {
	for index := range brands {
		if brands[index].Brand == "Chromium" {
